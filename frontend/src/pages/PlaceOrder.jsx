    ...
    const checkCodAvailability = () => {
      for (const itemId in cartItems) {
        ...
        if (product && product.codAvailable === false) {
          ...
          setMethod('razorpay'); // Default to Razorpay if COD is not available
          return;
        }
      }
          ...
          {Object.keys(cartItems).length > 0 ? (
            <div className='flex flex-col gap-4 mb-6'>
              {Object.keys(cartItems).map((itemId) => {
                ...
                return Object.keys(cartItems[itemId]).map((size, index) => {
                  const quantity = cartItems[itemId][size];
                  if (quantity <= 0) return null;
                  return (
                    <div key={`${itemId}-${size}-${index}`} className='flex items-center gap-4 border-b pb-2'>
                      <img
            ...
            <p className='text-gray-600 mb-6'>No items in cart.</p>
          )}
